...
{
  app_version     => '5.36.3.1', #BEWARE: do not use '.0.0' in the last two version digits
  bits            => 64,
  beta            => 0,
  ...
  build_job_steps => [
    ...
    ### NEXT STEP ###########################
    {
        plugin  => 'Perl::Dist::Strawberry::Step::BinaryToolsAndLibs',
        install_packages => {
            # Reinstate the old patch now.  Ideally we would get the perl core patching to work properly.
            'patch' => '<package_url>/kmx/64_tools/64bit_patch-2.5.9-7-bin_20100110_UAC.zip',
        },
    },
    ### NEXT STEP ###########################
